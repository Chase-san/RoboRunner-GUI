...
public class RobotScore {
	public static final Function<RobotScore, Double> NORMAL_SCORER = new Function<RobotScore, Double>() {
		@Override
		public Double apply(RobotScore robotScore) {
			return robotScore.score;
		}
	};
	public static final Function<RobotScore, Double> SURVIVAL_FIRSTS_SCORER = new Function<RobotScore, Double>() {
		@Override
		public Double apply(RobotScore robotScore) {
			return robotScore.survivalRounds;
		}
	};
	public static final Function<RobotScore, Double> SURVIVAL_SCORER = new Function<RobotScore, Double>() {
		@Override
		public Double apply(RobotScore robotScore) {
			return robotScore.survivalScore;
		}
	};
	public static final Function<RobotScore, Double> BULLET_DAMAGE_SCORER = new Function<RobotScore, Double>() {
		@Override
		public Double apply(RobotScore robotScore) {
	...
	public RobotScore(String botName, double score, double survivalRounds, double survivalScore, double bulletDamage,
			double energyConserved, int numBattles) {
	 ...
	 * 
	 * @param enemyScore
	 *            score data for the enemy robot
	 * @param numRounds
	 *            number of rounds in the battle
	 * @return the {@code RobotScore} relative to the given enemy robot score
	 */
	public RobotScore getScoreRelativeTo(RobotScore enemyScore, int numRounds) {
	 ...
	 * 
	 * @param enemyScore
	 *            score data for the other robots in the battle
	 * @param numRounds
	 *            number of rounds in the battle
	...
	private double getAverageScore(Function<RobotScore, Double> scorer, Collection<RobotScore> enemyScores) {
		...
		for (RobotScore robotScore : enemyScores) {
			totalScore += 100 * (challengerScore / (challengerScore + scorer.apply(robotScore)));
			numScores++;
		}
		return totalScore / numScores;
	}

	private double getAverageEnergyConserved(List<RobotScore> enemyScores, int numRounds) {
	...
	public enum ScoringStyle {
		...
		private ScoringStyle(String description, Function<RobotScore, Double> scorer, boolean isChallenge) {
