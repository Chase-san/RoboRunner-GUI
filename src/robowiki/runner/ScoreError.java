...
public class ScoreError {
	...
	public double getAccuracyGainRate() {
		return (getStandardError(numBattles) - getStandardError(numBattles + 1)) / avgTime;
	}

	public double generateRandomAverageScore() {
		double scoreTotal = 0;
		for (int x = 0; x < numBattles; x++) {
			scoreTotal += Math.max(0, Math.min(100, average + (RANDOM.nextGaussian() * standardDeviation)));
		}
